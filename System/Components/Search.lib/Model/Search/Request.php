...
class Model_Search_Request
{
	...
	public function addRequestElement($section, Model_Search_RequestElement $element){
		$this->assertSection($section);
		$this->data[$section][$element->getValue()] = $element;
		$this->stateChanged();
	}

	public function removeRequestElement($section, Model_Search_RequestElement $element){
		$this->assertElement($section, $element->getId());
		unset($this->data[$section][$element->getValue()]);
		$this->stateChanged();
	}

	public function hasElement($section, Model_Search_RequestElement $element){
		return (
	...
	public function hasDefinition($section, Model_Search_RequestElement $element){
		$this->assertSection($section);
	 ...
	 * @return array Model_Search_RequestElement[]
	 */
	...
	/**
	 * @return Model_Search_RequestElement
	 */
	...
	public function createRequestElement($value, $modifier = 0){
		return Model_Search_RequestElement::create($value, $modifier);
	}
	 ...
	 */
	public function apply(Model_Search_Request $request){
