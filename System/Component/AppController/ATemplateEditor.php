        ...
        IGlobalUniqueId  
{
    ...
    public function provideOpenDialogData(array $namedParameters)
    {
        ...
        $index = DFileSystem::FilesOf(SPath::TEMPLATES, '/\.tpl$/i');
        $items = array();
        foreach($index as $item)
        {
            $items[] = array(DFileSystem::name($item), $item, 0, filesize(SPath::TEMPLATES.$item), filemtime(SPath::TEMPLATES.$item));
        }
        $data = array(
